...
nish = np.array([8.0,7.0,1.0])
mo = np.array([3,10,0.98])
jun = np.array([10,5,0.96])
...
def win_lose_function(a,b,c,d,w):
    ...
    total_win_loss = np.empty(125000)
    for looper in range(total_win_loss.size):
        ...
        while trait < trait_weights.size:
            ...
            match_difference = match_difference + weighted_avg
...
print("The probability that Jon and Nish beat Mo and Jun is %f" %win_lose_function(jon,nish,mo,jun,trait_weights))
print("The probability that Mo and Nish beat Jon and Jun is %f" %win_lose_function(mo,nish,jon,jun,trait_weights))
print("The probability that Jon and Jun beat Mo and Nish is %f" %win_lose_function(jon,jun,mo,nish,trait_weights))
